...
import numpy as np
import torch
from config import label_map, rev_label_map
...
def decimate(tensor, m):
    """
    Decimate a tensor by a factor m, i.e. down sampled by keeping every m th value.
    This is used when we convert FC layers to equivalent Conv layers, but of a smaller size.
...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties, device):
    ...
    See https://medium.com/@jonathan_hui/map-mean-average-precision-for-object-detection-45c121a31173 for details
...
def xy_to_cxcy(xy):
    """
    Convert bounding boxes from (x_min, y_min, x_max, y_max) to center-size coordinates (c_x, c_y, w, h).
...
def cxcy_to_gcxgcy(cxcy, priors_cxcy):
    """
    Encode bounding boxes (that are in center-size form) with respect to
...
def adjust_learning_rate(optimizer, scale):
    ...
    print(f"The new learning rate is {optimizer.param_groups[1]['lr']}.")
